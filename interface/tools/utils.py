...
"""秒级别时间戳(10位数)转换成带格式的字符串 """
def timestamp10_to_str(timestamp, format_str="%Y-%m-%d %H:%M:%S"):
    time_array = time.localtime(timestamp)
    str_time = time.strftime(format_str, time_array)
    return str_time

"""毫秒级别时间戳(13位数)转换成带格式的字符串 """
def timestamp13_to_str(timestamp, format_str="%Y-%m-%d %H:%M:%S"):
    str_time = datetime.fromtimestamp(timestamp / 1000.0).strftime(format_str)
    return str_time

"""将"产品1,产品2;产品3"转换为[产品1,产品2,产品3] ， 即以英文逗号或英文分号作为分隔符将字符串切片为列表"""
def more_split(str_split):
    # str_split = "一二,三四,标准化产品-舆情秘书服务系统;人工推送服务"
    name_list = str_split.split(",")  # 将"产品1,产品2"转换为[产品1,产品2]
    name_list3 = []
    for name in name_list:
        name_list2 = name.split(";")
        for name2 in name_list2:
            name_list3.append(name2)
    return name_list3
