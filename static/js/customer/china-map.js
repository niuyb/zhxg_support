layui.use('layer', function(){ //独立版的layer无需执行这一句
    ...
    $.get("/customer/government/industry/coverage/api?industry2=46", function(data) {
        ...
        for (let item of data.items) {
            ...
            row.push(prov);
            row.push(item.base_count);
            row.push(item.formal_count + item.trial_count);
            // row.push(item.trial_count);
            rawData.push(row);
            // console.log(row);
        }
        ;
