...
(function(factory) {
    ...
    'use strict';
    var d = new Date(), doc = window.document, nowYear = d.getFullYear(), nowMonth = d.getMonth() + 1, domDate, createDate, time, body = $('body'), emptyStr = "<li></li>", isTouch = "ontouchend"in doc, tstart = isTouch ? "touchstart" : "mousedown", tmove = isTouch ? "touchmove" : "mousemove", tend = isTouch ? "touchend" : "mouseup", tcancel = isTouch ? "touchcancel" : "mouseleave", isEnglish = (navigator.language || navigator.browserLanguage).toLowerCase().indexOf('zh') == -1, h = 60, dpr = $('html').attr('data-dpr') || 1, resH = h , dateFormat = [['YYYY-MM-DD hh:mm:ss', 'YY-MM-DD hh:mm:ss', 'YYYY/MM/DD hh:mm:ss', 'YY/MM/DD hh:mm:ss'], ['YYYY-MM-DD hh:mm', 'YY-MM-DD hh:mm', 'YYYY/MM/DD hh:mm', 'YY/MM/DD hh:mm'], ['YYYY-MM-DD', 'YY-MM-DD', 'YYYY/MM/DD', 'YY/MM/DD'], ['YYYY-MM', 'YY-MM', 'YYYY/MM', 'YY/MM'], ['hh:mm:ss', 'hh:mm']], opts = {
        beginYear: 2010,
        endYear: 2088,
        type: 'YYYY-MM-DD',
        limitTime: false,
    ...
    createDate = {
        ...
        show: function(isShow) {
            ...
            if (isShow) {
                ...
                domMask.show();
                time = +new Date();
                domMain.css('overflow', 'hidden');
            } else {
                domMain.hide();
                domMask.hide();
                domMain.css('overflow', 'auto');
            }
        },
        ...
        toNow: function(refresh) {
            if (!refresh) {
                $('#date-wrapper ol').each(function() {
					...
					var liTop = -((that.children('.active').index() - 1) * h);
                    that.animate({
                        top: liTop
                    }, 600);
                })
    ...
    $('#d-confirm').bind('click', function() {
        var y = $('#d-year .active').text(), m = $('#d-month .active').text(), d = $('#d-day .active').text(), h = $('#d-hours .active').text(), min = $('#d-minutes .active').text(), s = $('#d-seconds .active').text(), str, that = $($(this).attr('d-id')), index = dateFormat[0].indexOf(opt.type), index1 = dateFormat[1].indexOf(opt.type), index2 = dateFormat[2].indexOf(opt.type), index3 = dateFormat[3].indexOf(opt.type), index4 = dateFormat[4].indexOf(opt.type);
        if (index != -1) {
            switch (index) {
            case 0:
                str = y + '-' + m + '-' + d + ' ' + h + ':' + min + ':' + s;
