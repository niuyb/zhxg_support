...
class Account(models.Model):
    ...
    crm_id = models.CharField(db_column='crm_id', max_length=255, blank=True, default="")
    update_timestamp = models.CharField(db_column='update_timestamp', max_length=255, blank=True)
...
#废掉调研表
class SystemRecommendSubject_bak(models.Model):
    ISMUST = (
        (0, "非必要"),
        (1, "必要")
    )
    ...
    catalog_name = models.CharField(db_column='catalog_name', max_length=64, blank=True, null=True)   # 分类名称
    catalog_id = models.IntegerField(db_column='catalog_id', blank=True, null=True)                 # 分类id
    subject_name = models.CharField(db_column='subject_name', max_length=64, blank=True, null=True)   # 专题名称
    industry_id = models.CharField(db_column='', max_length=64, blank=True, null=True)    # 二级行业id
    entity_tag_ids = models.TextField(db_column='entity_tag_ids',)  # This field type is a guess.      #主体标签id集合
    event_tag_ids = models.TextField(db_column='event_tag_ids',)  # This field type is a guess.     # 事件标签id集合
    ambiguous_words = models.TextField(db_column='ambiguous_words',)  # This field type is a guess.  #歧义词
    exclude_words = models.TextField(db_column='exclude_words',)  # This field type is a guess.      # 排除词
    precise_setting = models.TextField(db_column='precise_setting',)  # This field type is a guess.  #精准设置
    is_must = models.IntegerField(db_column='is_must',choices=ISMUST,blank=True, null=True)                   # 0 非必要 1 必要
    refer_count = models.IntegerField(db_column='refer_count',blank=True, null=True)           #引用次数
    type = models.IntegerField(db_column='type',choices=TYPE, blank=True, null=True)                 #类型，1 专题 2分类
    region_tag_type = models.IntegerField(db_column='region_tag_type',blank=True, null=True)   #1 地域标签 2 crm客户名称
    account_id = models.CharField(db_column='account_id',max_length=64)   #客户id
    create_at = models.DateTimeField(db_column='create_at', auto_now=True)              #创建时间
    user_id = models.CharField(db_column='user_id',max_length=64)         #用户id
    content = models.TextField(db_column='content')                      #具体答题内容
    queuuid = models.CharField(db_column="queuuid", max_length=36)

    class Meta:
        managed = False
        db_table = 'system_recommend_subject'
        app_label = ''

#答案表
label = 'survery' if settings.MODE == 'develop' or settings.MODE == 'beta' else 'secretary'
print('label:', label)
# label = 'secretary'

class Answer(models.Model):
    questionaire_id = models.CharField(max_length=255)
    content = models.TextField()
    subject_id = models.IntegerField()
    question_id = models.IntegerField()
    subject_name = models.CharField(max_length=255)
    account_id = models.CharField(max_length=255)
    customre_abbreviation = models.TextField()
    industry_id = models.IntegerField()
    region_tag_type = models.IntegerField()
...
class Question(models.Model):
    ...
    industry_id = models.CharField(max_length=64)
    is_must = models.IntegerField(choices=ISMUST)
...
#
class SurverySubject(models.Model):
    catalog_id = models.IntegerField(primary_key=True, db_column='catalog_id')
